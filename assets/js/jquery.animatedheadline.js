...
(function ($) {
	...
	animatedHeading.prototype.animateHeadline = function ($headlines) {
		...
		$headlines.each(function () {
			...
			if (headline.hasClass("loading-bar")) {
				...
				setTimeout(function () {
					headline.find(".qubely-animated-text .animated-text-words-wrapper").addClass("is-loading");
				}, self.barWaiting);
				...
				var words = headline.find(
					".qubely-animated-text .animated-text-words-wrapper .animated-text.is-visible"
				);
	...
	animatedHeading.prototype.hideWord = function ($word) {
		...
		if ($word.parents(".animated-heading-text").hasClass("type")) {
			...
			setTimeout(function () {
				$word.parents(".animated-text-words-wrapper").addClass("is-loading");
			}, self.barWaiting);
	...
	animatedHeading.prototype.showWord = function ($word, $duration) {
		...
		if ($word.parents(".animated-heading-text").hasClass("type")) {
			...
			$word
				...
				.animate({ width: $word.width() + 5 }, self.revealDuration, function () {
					setTimeout(function () {
						self.hideWord($word);
	...
	animatedHeading.prototype.showLetter = function ($letter, $word, $bool, $duration) {
		...
		if (!$letter.is(":last-child")) {
			...
			if ($word.parents(".animated-heading-text").hasClass("type")) {
				setTimeout(function () {
					$word.parents(".animated-text-words-wrapper").addClass("waiting");
				}, 200);
			}
			if (!$bool) {
				setTimeout(function () {
