(function ($) {
	"use strict";
	if (typeof qubelyInteraction === "undefined") return;
	/*
	 * ====================
	 * INTERACTION MATH
	 * ====================
	...
	function initiateTiltAnimation(windowWidth) {
		if (typeof qubelyInteraction["mouse_movement"] !== "undefined") {
			...
			mouseMovementList.map((tiltOpt) => {
				...
				if (
					windowWidth < 768 &&
					(typeof tiltOpt.animation.enable_mobile === "undefined" ||
						tiltOpt.animation.enable_mobile === false)
				) {
				...
				if (
					windowWidth > 767 &&
					windowWidth < 991 &&
					(typeof tiltOpt.animation.enable_tablet === "undefined" ||
						tiltOpt.animation.enable_tablet === false)
				) {
					if (typeof mouseTiltInstance[tiltOpt.blockId] !== "undefined") {
						mouseTiltInstance[tiltOpt.blockId].tiltIntance.destroy();
						delete mouseTiltInstance[tiltOpt.blockId];
	...
	if (typeof qubelyInteraction["while_scroll_view"] !== "undefined") {
		...
		$(document).ready(function () {
			scrollActionList.map((animationProperty) => {
				if (
					windowWidth < 768 &&
					(typeof animationProperty.enable_mobile === "undefined" ||
						animationProperty.enable_mobile === false)
				) {
					return false;
				}
				if (
					windowWidth > 767 &&
					windowWidth < 991 &&
					(typeof animationProperty.enable_tablet === "undefined" ||
						animationProperty.enable_tablet === false)
				) {
					return false;
				}
		...
		$(window).on("load", function () {
			$(document).on("scroll", function () {
				...
				scrollActionList.map((animationProperty) => {
					...
					if ($addonElem.parent().hasClass("qubley-interaction-wrapper")) {
						let scrollPercentage = percentageSeen(
							$addonElem.parent(".qubley-interaction-wrapper"),
							$window
						);
				...
				if (action.from !== null) {
					...
					if (typeof nextTriggerActions[key].from.x !== "undefined")
						nextTriggerActions[key].from.x =
							nextTriggerActions[key].from.x === "" || nextTriggerActions[key].from.x === null
								? 0
								: parseFloat(nextTriggerActions[key].from.x);
					if (typeof nextTriggerActions[key].from.y !== "undefined")
						nextTriggerActions[key].from.y =
							nextTriggerActions[key].from.y === "" || nextTriggerActions[key].from.y === null
								? 0
								: parseFloat(nextTriggerActions[key].from.y);
					if (typeof nextTriggerActions[key].from.z !== "undefined") {
						nextTriggerActions[key].from.z =
							nextTriggerActions[key].from.z === "" || nextTriggerActions[key].from.z === null
								? 0
								: parseFloat(nextTriggerActions[key].from.z);
					}
					if (typeof nextTriggerActions[key].from.value !== "undefined")
						nextTriggerActions[key].from.value =
							nextTriggerActions[key].from.value === ""
								? 0
								: parseFloat(nextTriggerActions[key].from.value);
				}
				if (action.to !== null) {
					...
					if (typeof nextTriggerActions[key].to.x !== "undefined") {
						nextTriggerActions[key].to.x =
							nextTriggerActions[key].to.x === "" || nextTriggerActions[key].to.x === null
								? 0
								: parseFloat(nextTriggerActions[key].to.x);
					...
					if (typeof nextTriggerActions[key].to.y !== "undefined") {
						nextTriggerActions[key].to.y =
							nextTriggerActions[key].to.y === "" || nextTriggerActions[key].to.y === null
								? 0
								: parseFloat(nextTriggerActions[key].to.y);
					...
					if (typeof nextTriggerActions[key].to.z !== "undefined") {
						nextTriggerActions[key].to.z =
							nextTriggerActions[key].to.z === "" || nextTriggerActions[key].to.z === null
								? 0
								: parseFloat(nextTriggerActions[key].to.z);
					...
					if (typeof nextTriggerActions[key].to.value !== "undefined") {
						nextTriggerActions[key].to.value =
							nextTriggerActions[key].to.value === "" || nextTriggerActions[key].to.value === null
								? 0
								: parseFloat(nextTriggerActions[key].to.value);
