jQuery(function ($) {
	...
	if (document.getElementsByClassName('qubely-table-of-contents').length > 0) {
		...
		tocOffsetTop =
			typeof tocOffsetTop !== 'undefined' && tocOffsetTop
				? parseInt(tocOffsetTop)
				: 0;
		...
		$('.qubely-table-of-contents-toggle a').on('click', function () {
			...
			parentElem
				.closest('.qubely-table-of-contents')
				.find('.qubely-table-of-contents-body')
				.slideToggle(300);
		});
			...
			function (e) {
				...
				if (itemToggle == 'true') {
					const $collapseItems = $accordion
						.find('.qubely-accordion-active')
						.not($selectedItem);
					if ($collapseItems.length) {
						$collapseItems
							.find('.qubely-accordion-body')
							.css('display', 'block')
							.slideUp(150);
	...
	$('.qubely-block-tab.with-auto-swithing').each(function () {
		...
		let changeTab = function () {
			...
			var $qubelyTab = $(
				'.qubely-tab-item:not(.qubely-backend):eq(' +
					idx +
					') .qubely-tab-title',
				currentElement,
			).parent();
			...
			progress(
				currentElement,
				$qubelyTab.find('.progress')[0],
				parseInt($qubelyTab[0].dataset.customdelay),
			);
			timeouts.push(
				setTimeout(changeTab, effectiveDelays[tabIndex % tabs] * 1000),
			);
		};
	...
	$('.qubely-vertical-tab-item-button').on('click', function (event) {
		...
		// nav
		$parentTab
			.find('.qubely-vertical-tab-item')
			.removeClass('qubely-vertical-active');
		...
		// body
		var $currentTabBody = $currentNav
			.closest('.qubely-vertical-tab-nav')
			.next();
		var $currentVerticalContent = $currentTabBody
			.find('.qubely-tab-content')
			.eq($currentNavIndex);
	...
	$('.qubely-carousel-title').on('click', function (event) {
		...
		$qubelyCarousel
			.closest('.qubely-carousel-nav-wraper')
			.find('.qubely-active')
			.removeClass('qubely-active');
	...
	$('.qubely-carousel-nav').on('click', function (event) {
		...
		let $qubelyCarouselBlock = $qubelyCarouselBody.closest(
			'.qubely-block-carousel',
		);
		let $nav = $(this);
		let direction = $nav.attr('data-direction');
		let items = $nav.attr('data-items');

		let activeItemlIndex = $(
			'.qubely-carousel-item-indicator.qubely-active',
		).index('.qubely-carousel-item-indicator');
		let nextActiveItem =
			direction === 'next'
		...
		$qubelyCarouselBlock
			...
			.removeClass('qubely-active');
		$qubelyCarouselBlock
			.find('.qubely-carousel-item-indicator.qubely-active')
			.removeClass('qubely-active');
		$qubelyCarouselBlock
			.find('.qubely-carousel-item')
			.eq(nextActiveItem)
		...
		$qubelyCarouselBlock
			.find('.qubely-carousel-item-indicator')
			.eq(nextActiveItem)
			.addClass('qubely-active');
	});
});
...
(function ($) {
	...
	$(document).on(
		...
		function (event, visible, visiblePartX, visiblePartY) {
			...
			if (visible) {
				...
				if (
					animation &&
					typeof animation.name != 'undefined' &&
					animation.openAnimation != 0
				) {
					setTimeout(() => {
						$this.css({ opacity: 1 });
					...
					$this.css({
						...
						'animation-iteration-count':
							animation.repeat === 'once' ? 1 : 'infinite',
					});
				}
			}
			$this.unbind('inview');
	...
	if (qubelySettingsTabContent.length) {
		if (window.location.hash) {
			qubelySettingTabs.removeClass('nav-tab-active');
			$('#qubely-settings-tabs a[href=' + window.location.hash + ']').addClass(
				'nav-tab-active',
			);
			qubelySettingsTabContent.find('.qubely-settings-inner').hide();
	...
	if (qubelyGsPostWrap.length) {
		function generatePostMarkup(post) {
			...
			if (
				...
				img.setAttribute(
					'src',
					post.qubely_featured_image_url.medium_large[0],
				);
			}
					...
					.then((res) => {
						if (Array.isArray(res)) {
							...
							localStorage.setItem(
								'__qubely_themeum_post_time',
								Date.now() + 3600e3,
							);
							localStorage.setItem(
								'__qubely_themeum_post',
								JSON.stringify(res),
							);
						} else {
			...
			fetch(endpoint, {
				...
				.then((response) => {
					response.json().then((response) => {
						var count =
							response.filter((item) => item.parentID === 0).length + '+';
