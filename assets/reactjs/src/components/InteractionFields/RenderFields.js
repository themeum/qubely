        ...
        return(
            <div className="qubely-animation-controller-fields">
                <div className="qubely-animation-control-keyframe">
                    <Range label={__("Timeline/Keyframe")} value={parseInt(keyframe)} onChange={(value) => this._onChangeProperties( 'keyframe', value )} min={0} max={100}/>
                </div>
                ...
                <div className="qubely-animation-control-property-fields">   
                        { Object.keys( property ).map( (key, index) => {
                            return <Range key={index} label={__(key)} value={parseFloat(property[key])} onChange={(value) => this._onChangeProperties( key, value, 'property' )} min={range.min} max={range.max} step={range.step}/>
                        })}
