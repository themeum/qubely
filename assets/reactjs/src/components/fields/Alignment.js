...
class Alignment extends Component {
    ...
    render() {
        ...
        const defData = this.props.disableJustify ? (this.props.disableCenter ? ['left', 'right'] : ['left', 'center', 'right']) : ['left', 'center', 'right', 'justify'];

        return (
            <div className={'qubely-field-alignment qubely-field ' + (this.props.responsive ? 'qubely-responsive' : '')} >
                ...
                <div className="qubely-field-button-list qubely-field-button-list-fluid">
                    {defData.map((data, index) => {
                        return (<button className={(this._filterValue() == data ? 'active' : '') + ' qubely-button'} key={index} onClick={() => this.setSettings(this._filterValue() == data ? '' : data)}>
