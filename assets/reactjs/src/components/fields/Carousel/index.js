	...
	render() {
		...
		return (
			<div
				...
				ref={(item) => (this.$node = $(findDOMNode(item)))}
				{...this.props.options}
			>
				...
				<div className="qubely-carousel-nav-control">
					{nav && (
						<Fragment>
							<span
								className={`next-control nav-control ${arrowPosition}`}
								onClick={() => this.$qubelyCarousel.navigate("next")}
							>
							...
							<span
								className={`prev-control nav-control ${arrowPosition}`}
								onClick={() => this.$qubelyCarousel.navigate("prev")}
							>
								{arrowStyle == "arrowright2" ? (
