		...
		return value.map(({ label }, index) => {
			return (
				<span key={index} className="qubely-dropdown-selected-value">
					<span className="qubely-selected-value-label" role="option" aria-selected="true">
						{" "}
						{label}{" "}
					</span>
					<span
						className="qubely-selected-value-icon"
						aria-hidden="true"
						onClick={() => {
	...
	return (
		<div className="qubely-field qubely-field-dropdown">
			...
			>
				...
				<span className="qubely-dropdown-icon" onClick={() => showOptions && toggleOptions(false)}>
					{" "}
					{showOptions ? icons.arrow_up : icons.arrow_down}{" "}
				</span>
			</div>
			{showOptions && (
				<div className="qubely-dropdown-options-wrapper" ref={qubelyOptionsWraper}>
					<div className="qubely-dropdown-options">
						{options.length > 0 ? (
							...
							<div className={`qubely-dropdown-option no-match`} onClick={() => toggleOptions(false)}>
								{" "}
								No matched Option{" "}
							</div>
						)}
