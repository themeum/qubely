		...
		return (
			<Fragment>
					...
					<InspectorTabs tabs={["style", "advance"]}>
						<InspectorTab key={"style"}>
							<PanelBody initialOpen={true} title={__("Dimension")}>
								<div
									className={
										columns === blockIndex + 1 && responsiveDevice === "md"
											? "qubely-mb-20 disable-slide"
											: "qubely-mb-20"
									}
								>
									<Range
							...
							<PanelBody initialOpen={true} title={__("Design")}>
								...
								<Border
									label={__("Border")}
									value={colBorder}
									onChange={(val) => setAttributes({ colBorder: val })}
									min={0}
									max={10}
								/>
								<Separator />
								<BoxShadow
									label={__("Box-Shadow")}
									value={colShadow}
									onChange={(val) => setAttributes({ colShadow: val })}
								/>
								<Separator />
								<BorderRadius
									label={__("Radius")}
				...
				{rowWidth !== 0 && (
					...
					>
						<div
							className={`qubely-column qubely-column-admin qubely-block-${uniqueId}${
								className ? ` ${className}` : ""
							}`}
							data-column-width={this.props.attributes.colWidth.md}
						>
