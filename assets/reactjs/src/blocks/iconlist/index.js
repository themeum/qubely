...
const attributes = {
	...
	iconColor: {
		...
		style: [{ selector: "{{QUBELY}} .qubely-list .qubely-list-li .qubely-list-item-icon {color: {{iconColor}};}" }],
	},
	iconHoverColor: {
		type: "string",
		default: "",
		style: [
			{
				selector:
					"{{QUBELY}} .qubely-list .qubely-list-li:hover .qubely-list-item-icon {color: {{iconHoverColor}};}",
			},
		],
	},
	...
	color: {
		type: "string",
		default: "#333",
		style: [{ selector: "{{QUBELY}} .qubely-list .qubely-list-li {color: {{color}};}" }],
	},
	colorHover: {
		type: "string",
		default: "",
		style: [{ selector: "{{QUBELY}} .qubely-list .qubely-list-li:hover {color: {{colorHover}};}" }],
	},
	spacing: {
		type: "string",
		default: "5",
	...
	shadowHover: {
		...
		style: [
			{
				condition: [{ key: "layout", relation: "==", value: "fill" }],
				selector: "{{QUBELY}} .qubely-list .qubely-list-li:hover",
			},
		],
	},
	...
	deprecated: [
		{
			attributes,
			save(props) {
				const { uniqueId, interaction } = props.attributes;

				const renderListItems = () => {
					const {
						attributes: { listItems, iconPosition },
					} = props;
					return listItems.map((item, index) => {
						return (
							<li key={index} className={"qubely-list-li"}>
								{iconPosition == "left" && (
									<span className={`qubely-list-item-icon ${item.icon} fa-fw`} />
								)}
				...
				return (
					<div className={`qubely-block-${uniqueId}`}>
						<div className={`qubely-block-icon-list ${interactionClass}`}>
							<ul className="qubely-list">{renderListItems()}</ul>
						</div>
					</div>
				);
			},
