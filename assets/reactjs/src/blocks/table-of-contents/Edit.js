...
const { PanelBody, TextControl, Toolbar, FormTokenField, CheckboxControl } = wp.components;

const { RichText, InspectorControls, BlockControls, BlockAlignmentToolbar } = wp.blockEditor;
		...
		return (
			<Fragment>
				...
				<div className={classes}>
					...
					>
						<div className={classnames(["qubely-table-of-contents-header", collapsibleAlignment])}>
							{showTitle && (
								...
								>
									{isCollapsed === true ? (
										collapsibleType !== "icon" ? (
											...
											<a href="javascript:;" className={currentIconClass.close}></a>
										)
									) : collapsibleType !== "icon" ? (
										<a href="javascript:;">{collapsibleClose}</a>
									) : (
										<a href="javascript:;" className={currentIconClass.open}></a>
									)}
								</div>
							)}
						</div>
