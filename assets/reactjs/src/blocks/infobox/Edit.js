...
class Edit extends Component {
    ...
    render() {
        ...
        if (uniqueId) { CssGenerator(this.props.attributes, 'infobox', uniqueId); }
            ...
            <Fragment>
                <InspectorControls key="inspector">
                    <PanelBody title=''>
                        ...
                        />
                        {(layout == 1 || layout == 4) &&
                            <Alignment label={__('Alignment')} value={alignment} alignmentType="content" onChange={val => setAttributes({ alignment: val })} alignmentType="content" disableJustify responsive />
                        }
                                ...
                                <Fragment>
                                    ...
                                    {mediaType == 'image' &&
                                        <Fragment>
                                            ...
                                            <Range label={__('Image Height')} value={imageHeight} onChange={val => setAttributes({ imageHeight: val })} min={0} max={500} unit={['px', 'em', '%']} responsive />
                                        </Fragment>
                                    }
                                    ...
                                    {mediaType == 'number' &&
                                        <Fragment>
                                            ...
                                            <Typography value={numberTypography} onChange={(value) => setAttributes({ numberTypography: value })} disableLineHeight />
                                        </Fragment>
                                    }
                                    ...
                                    <Toggle label={__('Background')} value={useMediaBg} onChange={val => setAttributes({ useMediaBg: val })} />
                                    {(useMediaBg == 1 && mediaType != 'number') &&
                                        <Fragment>
                                            <Range label={__('Background Size')} value={mediaBackgroundSize} onChange={val => setAttributes({ mediaBackgroundSize: val })} min={0} max={200} unit={['px', 'em', '%']} responsive />
                                            <BorderRadius label={__('Radius')} value={mediaBorderRadius} onChange={val => setAttributes({ mediaBorderRadius: val })} min={0} max={100} unit={['px', 'em', '%']} responsive />
                                            <Tabs>
                                                <Tab tabTitle={__('Normal')}>
                                                    <ColorAdvanced label={__('Background Color')} value={mediaBg} onChange={val => setAttributes({ mediaBg: val })} />
                                                    <Border label={__('Border')} value={mediaBorder} onChange={val => setAttributes({ mediaBorder: val })} min={0} max={10} unit={['px', 'em', '%']} responsive />
                                    ...
                                    <Range label={__('Spacing')} value={mediaSpacing} onChange={val => setAttributes({ mediaSpacing: val })} min={0} max={200} unit={['px', 'em', '%']} responsive />
                                </Fragment>
                            }
                        </PanelBody>
                    }
                    ...
                    <PanelBody title={__('Title')} opened={'Title' === openPanelSetting} onToggle={() => this.handlePanelOpenings(openPanelSetting !== 'Title' ? 'Title' : '')}>
                        ...
                        <Typography label={__('Typography')} value={titleTypography} onChange={(value) => setAttributes({ titleTypography: value })} />
                        <Range label={<span>Spacing <span className="dashicons dashicons-sort" title="Y Spacing" /></span>} value={titleSpacing} onChange={val => setAttributes({ titleSpacing: val })} min={0} max={200} unit={['px', 'em', '%']} responsive />
                        <Tabs>
                            <Tab tabTitle={__('Normal')}>
                        ...
                        {separatorStyle &&
                            <Fragment>
                                ...
                                <Range label={__('Width')} value={separatorWidth} onChange={val => setAttributes({ separatorWidth: val })} min={20} max={separators[separatorStyle].width} responsive />
                                <Range label={__('Spacing')} value={separatorSpacing} onChange={val => setAttributes({ separatorSpacing: val })} min={0} max={100} responsive />
                                <SelectControl
                                    label="Position"
                    ...
                    <PanelBody title={__('Sub Title')} opened={'Sub Title' === openPanelSetting} onToggle={() => this.handlePanelOpenings(openPanelSetting !== 'Sub Title' ? 'Sub Title' : '')}>
                        ...
                        {subTitle == 1 &&
                            <Fragment>
                                ...
                                <Typography label={__('Typography')} value={subTitleTypography} onChange={val => setAttributes({ subTitleTypography: val })} />
                                <Tabs>
                                    <Tab tabTitle={__('Normal')}>
                                ...
                                <Range label={__('Spacing')} value={subTitleSpacing} onChange={(value) => setAttributes({ subTitleSpacing: value })} unit={['px', 'em', '%']} min={0} max={100} responsive />
                            </Fragment>
                        }
                    </PanelBody>
                    ...
                    <PanelBody title={__('Content')} opened={'Content' === openPanelSetting} onToggle={() => this.handlePanelOpenings(openPanelSetting !== 'Content' ? 'Content' : '')}>
                        <Typography label={__('Typography')} value={contentTypography} onChange={(value) => setAttributes({ contentTypography: value })} /> 
                        <Tabs>
                            <Tab tabTitle={__('Normal')}>
                        ...
                        {enableButton == 1 &&
                            <Range label={__('Spacing')} value={contentSpacing} onChange={(value) => setAttributes({ contentSpacing: value })} unit={['px', 'em', '%']} min={0} max={100} responsive />
                        }
                    </PanelBody>
                    ...
                    <PanelBody title={__('Background')} initialOpen={false}>
                        <Tabs>
                            <Tab tabTitle={__('Normal')}>
                                ...
                                <Range label={<span className="dashicons dashicons-leftright" title="X Padding" />} value={bgPaddingX} onChange={val => setAttributes({ bgPaddingX: val })} min={0} max={200} unit={['px', 'em', '%']} responsive />
                                <Range label={<span className="dashicons dashicons-sort" title="Y Padding" />} value={bgPaddingY} onChange={val => setAttributes({ bgPaddingY: val })} min={0} max={200} unit={['px', 'em', '%']} responsive />
                                <Border label={__('Border')} value={bgBorder} onChange={val => setAttributes({ bgBorder: val })} min={0} max={10} unit={['px', 'em', '%']} responsive />
                                <BoxShadow label={__('Box-Shadow')} value={bgShadow} onChange={(value) => setAttributes({ bgShadow: value })} />
                                <BorderRadius label={__('Radius')} value={bgBorderRadius} onChange={(value) => setAttributes({ bgBorderRadius: value })} min={0} max={100} unit={['px', 'em', '%']} responsive />
                            </Tab>
                            <Tab tabTitle={__('Hover')}>
                ...
                <div className={`qubely-block-${uniqueId}`}>
                    <div className={`qubely-block-info-box qubely-info-box-layout-${layout}`}>
                        {(layout != 4 && mediaType) &&
                            <div className="qubely-info-box-media" onClick={() => this.handlePanelOpenings('Media')}>
