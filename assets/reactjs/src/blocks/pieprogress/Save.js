...
const Save = (props) => {
	...
	return (
		<div
			className={`qubely-block-${uniqueId} qubely-block-pie-progress ${interactionClass}`}
			...
			<div className="qubely-progress-parent">
				...
				{(enableIcon || enableHeading) && (
					<div className="qubely-progress-inner-text">
						{enableIcon && (
							<Fragment>
								{iconStyle === "text" && <RichText.Content tagName="div" value={iconText} />}
								{iconStyle === "percent" && (
									<div>
						...
						{enableHeading && headingPosition === "inside" && (
							<RichText.Content tagName="div" value={heading} className="qubely-pie-progress-heading" />
						)}
					</div>
				)}
			</div>
