...
class PageListModal extends Component {
	...
	render() {
		...
		return (
			<Fragment>
				<QubelyModal
					className="qubely-builder-modal-pages-list"
					customClass="qubely-builder-modal-template-list"
				...
				>
					<div className="qubely-builder-modal-header">
						<div className="template-search-box">
							...
							<input
								type="search"
								onChange={this._OnSearchTemplate.bind(this)}
						...
						<div className="qubely-template-list-header">
							<button
								className={this.state.itemType == "layout" ? "active" : ""}
								onClick={(e) => this._onlickLayoutsTab()}
							>
								{" "}
								{__("Starter Packs")}{" "}
							</button>
							<button
								className={this.state.itemType == "block" ? "active" : ""}
								onClick={(e) => this._onlickBlocksTab()}
							>
								{" "}
								{__("Sections")}{" "}
							</button>
							<button
								className={this.state.itemType == "saved_blocks" ? "active" : ""}
								onClick={(e) => this._onlickSavedBlocksTab()}
							>
								{" "}
								{__("Saved")}{" "}
							</button>
							...
							{((!this.state.parent_id && layer != "block") ||
								...
								<ul className="qubely-template-categories">
									...
									>
										...
										<span>{this._getDataLength("category", currentPageData.length)}</span>
									</li>
									{pageCategories.map((data, index) => (
										<li
											className={
												itemType == "block"
													? data.slug == selectedBlockCategory
														? "active"
														: ""
													: data.slug == selectedLayoutCategory
													? "active"
													: ""
											}
					...
					<div className="qubely-layout-modal-content-area">
						{itemType != "saved_blocks" && (
							<div className="qubely-template-list-sub-header">
								<h4>
									{this.state.itemType == "layout" && this.state.layer == "single" && (
										<span
											className={"qubely-template-back"}
											onClick={() => this.setState({ layer: "multiple", parent_id: "" })}
										>
							...
							<div id="modalContainer" className="qubely-template-list-modal">
								<div className="qubely-builder-template-list-container">
									...
									<div
										...
										className={
											"qubely-builder-page-templates " +
											(this.state.itemType == "saved_blocks" ? "qubely-frontendd-block-list" : "")
										}
									>
										{this.state.layer == "single" &&
										...
										{this.state.layer == "block" && currentPageData.length != 0 && (
											<div className="qubely-reusable-list-title">
												...
												<div className="qubely-reusable-list-info">
													<div className="qubely-reusable-list-info-date">
														{__("Publish")}{" "}
													</div>
												</div>
											</div>
										...
										{currentPageData.length === 0 && (
											<div className="qubely-builder-template-found-empty">
												<h3 className="qubely-builder-empty-title">
													{" "}
							...
							<div className="qubely-import-settings">
								...
								<div className="qubely-import-settings-footer">
									<div className="remember-choice-box">
										...
										/>
										<label for="isGoing" className="label">
