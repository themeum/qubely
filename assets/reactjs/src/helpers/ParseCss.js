  ...
  blocks.map((row) => {
    ...
    if (blockName[0] === "qubely" && attributes.uniqueId) {
      ...
      if (typeof attributes["interaction"] !== "undefined") {
        const { while_scroll_into_view, mouse_movement } =
          attributes.interaction;

        if (
          typeof while_scroll_into_view !== "undefined" &&
          while_scroll_into_view.enable === true
        ) {
  ...
  function recursive_block_map(blocks) {
    ...
    blocks.forEach((block) => {
      ...
      if (blockType === "qubely") {
        ...
        if (
          blocks_flag.interaction === false &&
          typeof attributes.interaction !== "undefined"
        ) {
          const { while_scroll_into_view, mouse_movement } =
            attributes.interaction;
          if (
...
const ParseCss = async (setDatabase = true) => {
      ...
      blocks.forEach((block) => {
        if (block.name.indexOf("core/block") != -1) {
          getReusableBlockCSS(block.attributes.ref).then(async (response) => {
            if (response.success) {
              const reuseableBlock = innerBlocks(
                wp.blocks.parse(response.data),
                true
              );
  ...
  localStorage.setItem(
    "qubelyInteraction",
    JSON.stringify(__blocks.interaction)
  );
