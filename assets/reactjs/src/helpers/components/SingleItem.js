...
export default (props) => {
	return (
		<div
			id={props.index == 0 ? "first-single-item" : ""}
			className={
				"qubely-single-block-item " + (props.types == "inactive" && props.data.pro == true ? "inactive" : "")
			}
		>
			<div className="qubely-single-item-inner">
				<div className="qubely-default-template-image">
					<img
						className="lazy"
						alt={__("Lazy Loading")}
						src={qubely_admin.plugin + "assets/img/image-loader.gif"}
						data-src={props.backgroundImage(props.data.image)}
					/>
				...
				<div className="qubely-tmpl-info">
					<div className="qubely-import-button-group">
						{props.itemType != "comming" ? (
							<Fragment>
								{props.data.liveurl && (
									<a className="qubely-button" target="_blank" href={props.data.liveurl}>
										<i className="fa fa-share" /> {__("Preview")}{" "}
									</a>
								)}
								{props.types == "inactive" && props.data.pro == true ? (
									<a
										className="qubely-button-download"
										target="_blank"
									...
									<a
										className="qubely-button qubely-button-download"
										onClick={(e) => {
											props.importLayoutBlock(props.data, props.data.pro);
										}}
									>
										{props.spinner == props.data.ID ? (
							...
							<div className="qubely-coming-soon" style={{ color: "#ffffff" }}>
								{__("Coming Soon.")}
							</div>
						)}
					</div>
